...
public class ResourceAnalyticsView extends VerticalLayout implements BeforeEnterObserver {
    ...
    public ResourceAnalyticsView(AnalyticsService analyticsService) {
        ...
        add(
                new com.vaadin.flow.component.html.Span(
                        "Total Resources: " + stats.get("totalResources")));
        add(
                new com.vaadin.flow.component.html.Span(
                        "Total Reservations: " + stats.get("totalReservations")));
        add(
                new com.vaadin.flow.component.html.Span(
                        "Approved Reservations: " + stats.get("approvedReservations")));
        add(
                new com.vaadin.flow.component.html.Span(
                        "Pending Reservations: " + stats.get("pendingReservations")));
        add(
                new com.vaadin.flow.component.html.Span(
                        "Active Reservations: " + stats.get("activeReservations")));
        add(new H3("Top 5 Most Used Resources:"));
        @SuppressWarnings("unchecked")
        java.util.List<Map.Entry<String, Long>> topResources =
                (java.util.List<Map.Entry<String, Long>>) stats.get("topResources");
        if (topResources != null && !topResources.isEmpty()) {
            topResources.forEach(
                    entry -> {
                        add(
                                new com.vaadin.flow.component.html.Span(
                                        entry.getKey()
