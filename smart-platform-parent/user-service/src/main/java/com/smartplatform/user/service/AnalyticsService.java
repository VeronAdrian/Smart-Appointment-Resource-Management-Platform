...
public class AnalyticsService {
    ...
    // Appointment Trends
    public static Map<String, Object> getAppointmentTrends() {
        ...
        long upcoming = allAppointments.stream()
                ...
                .filter(apt -> apt.getAppointmentDateTime().toLocalDate().isBefore(today.plusDays(8)))
                .count();
    ...
    // Staff Utilization
    public static Map<String, Object> getStaffUtilization() {
        ...
        allSlots.forEach(slot -> {
            ...
            staffStats.get(staff).put("totalSlots", 
                staffStats.get(staff).getOrDefault("totalSlots", 0L) + 1);
            if (slot.isBooked()) {
                staffStats.get(staff).put("bookedSlots",
                    staffStats.get(staff).getOrDefault("bookedSlots", 0L) + 1);
            }
        });
        ...
        staffStats.forEach((staff, stats) -> {
            ...
            staffData.put("utilizationPercent", Math.round(utilizationPercent * 10.0) / 10.0);
    ...
    // Resource Occupancy
    public static Map<String, Object> getResourceOccupancy() {
        ...
        long totalActive = resourceOccupancy.stream()
                .mapToLong(r -> (Long) r.get("activeReservations"))
                .sum();
    ...
    public static Map<String, Object> getRevenueInsights() {
        Map<String, Object> revenue = new HashMap<>();
        List<Appointment> allAppointments = AppointmentService.getAllAppointments();
        List<ResourceReservation> allReservations = ResourceService.getAllReservations();
        
        // Mock revenue calculation (assume $50 per appointment, $30 per resource reservation)
        long confirmedAppointments = allAppointments.stream()
                .filter(apt -> apt.getStatus().equals("CONFIRMED") || apt.getStatus().equals("PENDING"))
                .count();
