...
/**
 * REST Controller for Search and Recommendation endpoints Provides fast searching for appointments
 * and resources using Elasticsearch Provides intelligent recommendations for best slots and
 * resources
 */
@RestController
...
public class SearchController {
    ...
    @Autowired private ResourceSearchService resourceSearchService;
    ...
    /** Search appointments by client GET /api/search/appointments/client/{clientUsername} */
    @GetMapping("/appointments/client/{clientUsername}")
    public ResponseEntity<Map<String, Object>> searchAppointmentsByClient(
            @PathVariable String clientUsername) {
        String tenantId = tenantContext.getCurrentTenantId();
        List<AppointmentDocument> results =
                appointmentSearchService.searchByClient(clientUsername, tenantId);

        return ResponseEntity.ok(
                Map.of("status", "success", "count", results.size(), "data", results));
    }

    /** Search appointments by staff member GET /api/search/appointments/staff/{staffUsername} */
    @GetMapping("/appointments/staff/{staffUsername}")
    public ResponseEntity<Map<String, Object>> searchAppointmentsByStaff(
            @PathVariable String staffUsername) {
        String tenantId = tenantContext.getCurrentTenantId();
        List<AppointmentDocument> results =
                appointmentSearchService.searchByStaff(staffUsername, tenantId);

        return ResponseEntity.ok(
                Map.of("status", "success", "count", results.size(), "data", results));
    }

    /** Search appointments by status GET /api/search/appointments/status/{status} */
    @GetMapping("/appointments/status/{status}")
    public ResponseEntity<Map<String, Object>> searchAppointmentsByStatus(
            @PathVariable String status) {
        String tenantId = tenantContext.getCurrentTenantId();
        List<AppointmentDocument> results =
                appointmentSearchService.searchByStatus(status, tenantId);

        return ResponseEntity.ok(
                Map.of("status", "success", "count", results.size(), "data", results));
    }

    /** Search appointments by type GET /api/search/appointments/type/{type} */
    ...
    /**
     * Search appointments by date range GET
     * /api/search/appointments/date-range?startDate=2024-01-01T00:00:00&endDate=2024-01-31T23:59:59
     */
    ...
    public ResponseEntity<Map<String, Object>> searchAppointmentsByDateRange(
        ...
        String tenantId = tenantContext.getCurrentTenantId();
        List<AppointmentDocument> results =
                appointmentSearchService.searchByDateRange(startDate, endDate, tenantId);
    ...
    /**
     * Search staff schedule within a date range GET
     * /api/search/appointments/staff-schedule/{staffUsername}?startDate=2024-01-01T00:00:00&endDate=2024-01-31T23:59:59
     */
    ...
    public ResponseEntity<Map<String, Object>> searchStaffSchedule(
        ...
        String tenantId = tenantContext.getCurrentTenantId();
        List<AppointmentDocument> results =
                appointmentSearchService.searchStaffSchedule(
    ...
    /** Search resources by type GET /api/search/resources/type/{type} */
    ...
    /** Search available resources GET /api/search/resources/available */
    ...
    /** Search resources by name or description GET /api/search/resources/search?term=meeting */
    ...
    public ResponseEntity<Map<String, Object>> searchResourcesByNameOrDescription(
        ...
        List<ResourceDocument> results =
                resourceSearchService.searchByNameOrDescription(term, tenantId);

        return ResponseEntity.ok(
                Map.of("status", "success", "count", results.size(), "data", results));
    }

    /** Get high-rated resources GET /api/search/resources/high-rated */
    ...
    /** Get least used resources GET /api/search/resources/least-used */
    ...
    /**
     * Get recommended appointment slots GET
     * /api/search/recommendations/slots?appointmentType=consultation&durationMinutes=30&startDate=2024-01-15T00:00:00&endDate=2024-01-31T23:59:59&maxResults=5
     */
    ...
    public ResponseEntity<RecommendationResponse> getRecommendedSlots(
        ...
        List<SlotRecommendation> recommendations =
                recommendationEngine.recommendSlots(
        ...
        response.setSearchCriteria(
                String.format(
                        "appointmentType=%s, duration=%d min", appointmentType, durationMinutes));
    ...
    /**
     * Get recommended resources GET
     * /api/search/recommendations/resources?resourceType=meetingRoom&location=floor1&startDate=2024-01-15T00:00:00&endDate=2024-01-31T23:59:59&maxResults=5
     */
    ...
    public ResponseEntity<RecommendationResponse> getRecommendedResources(
        ...
        List<ResourceRecommendation> recommendations =
                recommendationEngine.recommendResources(
        ...
        response.setSearchCriteria(
                String.format("resourceType=%s, location=%s", resourceType, location));
        response.setTotalResults(recommendations.size());
        response.setExecutionTimeMs(executionTime);
    ...
    /**
     * Get combined recommendations for appointments and resources GET
     * /api/search/recommendations/combined?appointmentType=consultation&resourceType=meetingRoom&durationMinutes=30&startDate=2024-01-15T00:00:00&endDate=2024-01-31T23:59:59
     */
    ...
    public ResponseEntity<RecommendationResponse> getCombinedRecommendations(
        ...
        List<SlotRecommendation> slotRecommendations =
        ...
        List<ResourceRecommendation> resourceRecommendations =
                resourceType != null
                        ? recommendationEngine.recommendResources(
                                resourceType, null, startDate, endDate, tenantId, maxResults)
                        : List.of();
        ...
        response.setSearchCriteria(
                String.format(
                        "appointmentType=%s, resourceType=%s", appointmentType, resourceType));
