...
/**
 * Service for searching resources using Elasticsearch
...
public class ResourceSearchService {
     ...
     */
    public List<ResourceDocument> searchByTypeAndAvailability(String type, boolean available, String tenantId) {
        return resourceSearchRepository.findByTypeAndIsAvailableAndTenantId(type, available, tenantId);
    }
     ...
     */
    public List<ResourceDocument> advancedSearch(String type, String location, boolean availableOnly, 
                                                  String searchTerm, String tenantId, int page, int size) {
        List<ResourceDocument> results = resourceSearchRepository.findAll().stream()
                ...
                .filter(res -> searchTerm == null || 
                        res.getName().toLowerCase().contains(searchTerm.toLowerCase()) ||
                        res.getDescription().toLowerCase().contains(searchTerm.toLowerCase()))
