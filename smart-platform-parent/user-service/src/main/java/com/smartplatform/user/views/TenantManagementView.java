...
public class TenantManagementView extends VerticalLayout implements BeforeEnterObserver {
    public TenantManagementView() {
        ...
        Button createButton = new Button("Create Tenant", e -> {
            if (nameField.isEmpty() || domainField.isEmpty() || emailField.isEmpty()) {
        ...
        if (tenants.isEmpty()) {
            ...
            tenants.forEach(tenant -> {
                ...
                layout.add(new com.vaadin.flow.component.html.Span(
                    tenant.getName() + " (" + tenant.getDomain() + ") - Status: " + tenant.getStatus()
                ));
                ...
                statusCombo.addValueChangeListener(e -> {
                    if (TenantService.updateTenantStatus(tenant.getId(), e.getValue())) {
