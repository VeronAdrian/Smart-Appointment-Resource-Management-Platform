...
public class ResourceReservationView extends VerticalLayout implements BeforeEnterObserver {
    public ResourceReservationView() {
        ...
        List<Resource> availableResources = ResourceService.getAllResources().stream()
        ...
        Button reserveButton = new Button("Reserve", e -> {
            ...
            LocalTime startTime = LocalTime.of(
                ...
                startMinute.getValue() != null ? startMinute.getValue() : 0
            );
            LocalDateTime startDateTime = LocalDateTime.of(date, startTime);
            LocalDateTime endDateTime = startDateTime.plusHours(durationHours.getValue() != null ? durationHours.getValue() : 1);
            ResourceReservation reservation = ResourceService.createReservation(
                resource.getId(),
                userId,
                userId,
                startDateTime,
            ...
            if (reservation != null) {
                Notification.show("Reservation created! Pending approval. Check console for email notification.");
                refreshMyReservations(userId);
            } else {
                Notification.show("Failed to create reservation. Conflict detected or invalid time.");
            }
        });
        add(resourceCombo, datePicker, startHour, startMinute, durationHours, priorityCombo, notesField, reserveButton);
        ...
        if (reservations.isEmpty()) {
            ...
            reservations.forEach(reservation -> {
                ...
                layout.add(new com.vaadin.flow.component.html.Span(
                    ...
                    reservation.getStartDateTime().toString() + " - " + reservation.getEndDateTime().toString() +
                    " | Priority: " + reservation.getPriority() + " | Status: " + reservation.getStatus()
                ));
                if (reservation.getStatus().equals("PENDING") || reservation.getStatus().equals("APPROVED")) {
                    Button cancelButton = new Button("Cancel", e -> {
                        if (ResourceService.cancelReservation(reservation.getId())) {
