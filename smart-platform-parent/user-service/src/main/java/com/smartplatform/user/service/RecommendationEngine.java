 ...
 * recommendations for: 1. Best available appointment slots based on staff
 * ratings and availability.
 * 2. Best available resources based on ratings, usage patterns, and
 * availability.
 */
@Service
public class RecommendationEngine {
    ...
    /**
     * Recommend best available slots for an appointment. Scoring factors: Staff
     * member rating
     * (40%), Slot availability (30%), Appointment duration fit (20%), User
     * preferences (10%).
     */
    public List<SlotRecommendation> recommendSlots(
        ...
        List<AvailabilitySlot> availableSlots = availabilityService.getAvailableSlotsInRange(
                preferredStart, preferredEnd, tenantId);
        ...
        List<SlotRecommendation> recommendations = availableSlots.stream()
                ...
                .sorted(
                        Comparator.comparingDouble(
                                SlotRecommendation::getRecommendationScore)
                                .reversed())
    ...
    /**
     * Recommend best resources based on multiple criteria. Scoring factors:
     * Resource rating (35%),
     * Low utilization (30%) - less busy = better experience, Type match (20%),
    ...
    public List<ResourceRecommendation> recommendResources(
            String resourceType,
            String location,
            LocalDateTime requiredFrom,
            LocalDateTime requiredUntil,
            String tenantId,
            int maxRecommendations) {

        // Get available resources of the specified type
        List<ResourceDocument> availableResources = resourceSearchService.searchByTypeAndAvailability(resourceType,
                true, tenantId);
        ...
        List<ResourceRecommendation> recommendations = availableResources.stream()
                ...
                .sorted(
                        Comparator.comparingDouble(
                                ResourceRecommendation::getRecommendationScore)
                                .reversed())
                .limit(maxRecommendations)
                .collect(Collectors.toList());
    ...
    private SlotRecommendation scoreSlot(
            AvailabilitySlot slot, String appointmentType, int requestedDuration, String tenantId) {
        ...
        recommendation.setStartTime(slot.getStartDateTime());
        recommendation.setEndTime(slot.getEndDateTime());
    ...
    private ResourceRecommendation scoreResource(
            ResourceDocument resource, String preferredLocation, String tenantId) {
    ...
    private double calculateStaffRatingScore(String staffUsername, String tenantId) {
        List<AppointmentDocument> staffAppointments = appointmentSearchService.getHighRatedAppointments(staffUsername,
                tenantId);
    ...
    private double calculateAvailabilityScore(AvailabilitySlot slot) {
        ...
        int slotDuration = (int) java.time.temporal.ChronoUnit.MINUTES.between(
    ...
    private double calculateTimeProximityScore(AvailabilitySlot slot) {
        ...
        long minutesFromNow = java.time.temporal.ChronoUnit.MINUTES.between(now, slot.getStartDateTime());
