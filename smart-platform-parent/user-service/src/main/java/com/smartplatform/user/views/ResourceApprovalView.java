...
public class ResourceApprovalView extends VerticalLayout implements BeforeEnterObserver {
    ...
    public ResourceApprovalView(ResourceService resourceService) {
                    ...
                    reservation -> {
                        ...
                        Button approveButton =
                                new Button(
                                        "Approve",
                                        e -> {
                                            if (resourceService.approveReservation(
                                                    reservation.getId(), approverName)) {
                                                Notification.show(
                                                        "Reservation approved! Check console for"
                                                                + " email notification.");
                                                refreshPendingReservations(approverName);
                                            }
                                        });
                        ...
                        Button rejectButton =
                                new Button(
                                        ...
                                        e -> {
                                            ...
                                            if (resourceService.rejectReservation(
                                                    reservation.getId(),
                                                    approverName,
                                                    rejectReason.getValue())) {
                                                Notification.show(
                                                        "Reservation rejected! Check console for"
                                                                + " email notification.");
                                                refreshPendingReservations(approverName);
                                            }
                                        });
