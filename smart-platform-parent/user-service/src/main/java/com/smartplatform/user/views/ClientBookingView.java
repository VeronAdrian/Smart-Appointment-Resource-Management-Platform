...
public class ClientBookingView extends VerticalLayout implements BeforeEnterObserver {
    ...
    public ClientBookingView(
        ...
        if (auth == null
                || auth.getAuthorities().stream()
                        .noneMatch(a -> a.getAuthority().equals("CLIENT"))) {
            return;
        }
    ...
    private void refreshAvailableSlots(String clientUsername) {
                        ...
                        Button bookButton =
                                new Button(
                                        ...
                                        e -> {
                                            ...
                                            if (appointment != null) {
                                                Notification.show(
                                                        "Appointment booked successfully! Check"
                                                                + " console for email/SMS"
                                                                + " notification.");
                                                refreshAvailableSlots(clientUsername);
                                            } else {
                                                Notification.show(
                                                        "Failed to book appointment. Slot may have"
                                                                + " been taken.");
                                                refreshAvailableSlots(clientUsername);
                                            }
                                        });
        ...
        List<Appointment> myAppointments =
                appointmentService.getAppointmentsByClient(clientUsername);
                                    ...
                                    new Button(
                                            ...
                                            e -> {
                                                if (appointmentService.cancelAppointment(
                                                        apt.getId())) {
                                                    Notification.show(
                                                            "Appointment cancelled. Check console"
                                                                    + " for notification.");
                                                    refreshAvailableSlots(clientUsername);
                                                }
                                            });
