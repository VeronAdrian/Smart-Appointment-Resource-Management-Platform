...
/**
 * Service for searching appointments using Elasticsearch
...
public class AppointmentSearchService {
    ...
    public List<AppointmentDocument> searchByClient(String clientUsername, String tenantId) {
        return appointmentSearchRepository.findByClientUsernameAndTenantId(clientUsername, tenantId);
    }
    ...
    public List<AppointmentDocument> searchByType(String appointmentType, String tenantId) {
        return appointmentSearchRepository.findByAppointmentTypeAndTenantId(appointmentType, tenantId);
    }
    
    /**
     * Search appointments within a date range
     */
    public List<AppointmentDocument> searchByDateRange(LocalDateTime startDate, LocalDateTime endDate, String tenantId) {
     ...
     */
    public List<AppointmentDocument> searchStaffSchedule(String staffUsername, LocalDateTime startDate, 
                                                          LocalDateTime endDate, String tenantId) {
        return appointmentSearchRepository.findByStaffAndDateRange(staffUsername, startDate, endDate, tenantId);
    }
    
    /**
     * Get high-rated appointments for a client
     */
    public List<AppointmentDocument> getHighRatedAppointments(String clientUsername, String tenantId) {
     ...
     */
    public List<AppointmentDocument> advancedSearch(String clientUsername, String status, 
                                                     LocalDateTime startDate, LocalDateTime endDate, 
                                                     String tenantId, int page, int size) {
        List<AppointmentDocument> results = appointmentSearchRepository.findByDateRangeAndTenant(startDate, endDate, tenantId);
        
        // Filter by additional criteria
        results = results.stream()
                .filter(apt -> clientUsername == null || apt.getClientUsername().equals(clientUsername))
